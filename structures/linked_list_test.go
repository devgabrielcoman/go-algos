...
func Test_LinkedList(t *testing.T) {
	t.Run("can create a new linked list", func(t *testing.T) {
	...
	t.Run("can add at index", func(t *testing.T) {
		linkedList := structures.LinkedList[int]()
		linkedList.AddAtIndex(0, 8)
		assert.Equal(t, []int{8}, linkedList.ToArray())
		linkedList.AddAtStart(9)
		linkedList.AddAtStart(12)
		assert.Equal(t, []int{12, 9, 8}, linkedList.ToArray())
		linkedList.AddAtIndex(1, 5)
		assert.Equal(t, []int{12, 5, 9, 8}, linkedList.ToArray())
		linkedList.AddAtIndex(0, 22)
		assert.Equal(t, []int{22, 12, 5, 9, 8}, linkedList.ToArray())
		linkedList.AddAtIndex(-5, 20)
		assert.Equal(t, []int{22, 12, 5, 9, 8}, linkedList.ToArray())
	})

	t.Run("can remove at index", func(t *testing.T) {
		linkedList := structures.LinkedList[int]()
		res1 := linkedList.RemoveAtIndex(-1)
		assert.Nil(t, res1)
		...
		res2 := linkedList.RemoveAtIndex(1)
		assert.Equal(t, 5, res2.Value)
		assert.Equal(t, []int{7, 3}, linkedList.ToArray())

		res3 := linkedList.RemoveAtIndex(0)
		assert.Equal(t, 7, res3.Value)
		assert.Equal(t, []int{3}, linkedList.ToArray())

		res4 := linkedList.RemoveAtIndex(5)
		assert.Nil(t, res4)
		assert.Equal(t, []int{3}, linkedList.ToArray())

		res5 := linkedList.RemoveAtIndex(0)
		assert.Equal(t, 3, res5.Value)
		assert.Equal(t, []int{}, linkedList.ToArray())

		linkedList.AddAtEnd(10)
		linkedList.AddAtEnd(11)
		linkedList.AddAtEnd(12)
		res6 := linkedList.RemoveAtIndex(2)
		assert.Equal(t, 12, res6.Value)
	})
}
